FROM python:3.11-slim

LABEL maintainer="fraud-detection-team"
LABEL description="Credit Card Fraud Detection API"

RUN apt-get update && apt-get install -y --no-install-recommends \
    gcc g++ curl && rm -rf /var/lib/apt/lists/*

WORKDIR /app

COPY requirements.txt .
RUN pip install --no-cache-dir -r requirements.txt

COPY src/ ./src/
COPY notebooks/saved_models/ ./notebooks/saved_models/
COPY model_registry/ ./model_registry/

ENV PYTHONPATH=/app
ENV MODEL_REGISTRY_DIR=/app/model_registry
ENV SAVED_MODELS_DIR=/app/notebooks/saved_models
ENV DEFAULT_MODEL=xgboost
ENV FRAUD_THRESHOLD=0.5

EXPOSE 8000

HEALTHCHECK --interval=30s --timeout=5s --start-period=10s --retries=3 \
  CMD curl -f http://localhost:8000/health || exit 1

CMD ["uvicorn", "src.api:app", "--host", "0.0.0.0", "--port", "8000", \
     "--workers", "2", "--loop", "uvloop", "--access-log"]
